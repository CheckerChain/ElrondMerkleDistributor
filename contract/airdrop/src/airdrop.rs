...
pub trait Airdrop {
  ...
  fn claim(
    ...
    require!(!self.claim_status(&caller, &epoch_number).get(), "User Already Claimed!");
    ...
    // verify proof
    require!(self.verify(proof, root, leaf_hash), "Proof Missmatched Or Data Missmatch!");
    ...
    // send airdrop
    self.send()
      .direct_esdt(&caller, &token_id, 0, &amount);
  }
    ...
    fn verify(
        ...
        if &computed_hash.to_byte_array() <= &proof_element.to_byte_array() {
            computed_hash = self.efficient_hash(&computed_hash.as_managed_buffer(), &proof_element.as_managed_buffer());
        }
        // Hash(current element of the proof + current computed hash)
        else{
            computed_hash = self.efficient_hash(&proof_element.as_managed_buffer(), &computed_hash.as_managed_buffer());
        }
        }
    ...
    fn efficient_hash(&self, a: &ManagedBuffer, b: &ManagedBuffer) -> ManagedByteArray<Self::Api, HASH_LENGTH> {
    ...
    #[endpoint]
    #[only_owner]
    fn set_epoch_root(&self, epoch_number: BigUint, merkle_root:ManagedByteArray<Self::Api, HASH_LENGTH>){
    ...
    fn epoch_root(&self, epoch_number: &BigUint) -> SingleValueMapper<ManagedByteArray<Self::Api, HASH_LENGTH>>;

    #[view(getClaimStatus)]
    #[storage_mapper("claimStatus")]
    fn claim_status(&self, user: &ManagedAddress, epoch_number: &BigUint) -> SingleValueMapper<bool>;
